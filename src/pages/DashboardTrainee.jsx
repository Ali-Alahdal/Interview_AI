...
import { Play, BookOpen, Code2, Database, Briefcase, ChevronRight, Loader2 } from "lucide-react";
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
                            ...
                            <div key={course.id} className="glass-card rounded-2xl overflow-hidden hover:shadow-xl hover:shadow-primary/5 transition-all duration-300 group flex flex-col border border-border hover:border-primary/20">
                                    ...
                                    <div className="p-6 flex-1 flex flex-col">
                                        <span className="text-xs font-bold text-primary uppercase tracking-wider bg-primary/10 px-2 py-1 rounded-md w-fit mb-3">
                                            {course.specialty || "Genel Hazırlık"}
                                        </span>

                                        <h3 className="text-base font-bold text-foreground mb-2 line-clamp-2 group-hover:text-primary transition-colors">{course.title}</h3>
                                        <p className="text-sm text-muted-foreground line-clamp-2 mb-5 flex-1">
                                            {course.description || "Açıklama bulunmuyor."}
                                        </p>

                                        <Button variant="ghost" size="sm" onClick={() => navigate(`/study/${course.id}`)} className="text-primary hover:text-primary hover:bg-primary/10 font-bold w-full justify-between mt-auto border border-primary/20">
                                            Eğitime Başla <ChevronRight className="w-4 h-4 ml-1" />
                                        </Button>
                                    </div>
                                </div>
                            ))}
